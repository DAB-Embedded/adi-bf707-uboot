...
#else
	u32 conf = FRAMEBURSTENABLE | FES_100 | CRC_STRIP;
#endif
...
#ifdef CONFIG_BLACKFIN
	priv->txbuffs = (char *)L1_DATA_A_SRAM;
	priv->rxbuffs = priv->txbuffs + TX_TOTAL_BUFSIZE;
	priv->tx_mac_descrtable = (struct dmamacdescr *)(priv->rxbuffs +
			RX_TOTAL_BUFSIZE);
	priv->rx_mac_descrtable = (struct dmamacdescr *)
			(priv->tx_mac_descrtable + CONFIG_TX_DESCR_NUM *
			sizeof(struct dmamacdescr));
#endif
